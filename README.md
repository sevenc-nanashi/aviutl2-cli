...
> [!WARNING]
> このツールはデフォルトではシンボリックリンクを使用してファイルを配置します。
...
このAviUtl2は通常のAviUtl2とは違い、以下の特徴があります：

- `data` ディレクトリが自動的に作成されます。
- 以下の設定がデフォルトで適用されます：
  - 編集レジューム。
  - ログウィンドウの表示。
  - ログファイルのレベル・ログウィンドウのレベルが `VERBOSE` に設定されます。
...
HTTP の成果物を再取得する場合は `--refresh` を指定します。
AviUtl2の起動後は開発用ディレクトリの `data/log` にある最新のログを追尾します。
ログを追尾せずに終了する場合は `--detach` を指定します。
...
AviUtl2の設定の一部をリセットします。
現在、カスタムオブジェクト・エフェクトの設定と移動方法の設定をリセットすることができます。
